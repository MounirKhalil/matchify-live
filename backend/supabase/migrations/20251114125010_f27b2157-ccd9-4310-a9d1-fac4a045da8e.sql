...
CREATE INDEX IF NOT EXISTS idx_headhunt_searches_recruiter ON public.headhunt_searches(recruiter_id);
CREATE INDEX IF NOT EXISTS idx_headhunt_searches_status ON public.headhunt_searches(search_status);
CREATE INDEX IF NOT EXISTS idx_external_candidates_source ON public.external_candidates(source);
CREATE INDEX IF NOT EXISTS idx_external_candidates_search ON public.external_candidates(found_by_search_id);
CREATE INDEX IF NOT EXISTS idx_external_candidates_fingerprint ON public.external_candidates(fingerprint);
CREATE INDEX IF NOT EXISTS idx_scraping_jobs_search ON public.scraping_jobs(search_id);
...
CREATE POLICY "Recruiters can view their own headhunt searches"
  ON public.headhunt_searches FOR SELECT
  USING (
    recruiter_id IN (
...
CREATE POLICY "Recruiters can create their own headhunt searches"
  ON public.headhunt_searches FOR INSERT
  WITH CHECK (
    recruiter_id IN (
      SELECT id FROM public.recruiter_profiles WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Recruiters can update their own headhunt searches"
  ON public.headhunt_searches FOR UPDATE
  USING (
    recruiter_id IN (
      SELECT id FROM public.recruiter_profiles WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Recruiters can delete their own headhunt searches"
  ON public.headhunt_searches FOR DELETE
  USING (
    recruiter_id IN (
      SELECT id FROM public.recruiter_profiles WHERE user_id = auth.uid()
    )
...
CREATE POLICY "Recruiters can view external candidates from their searches"
  ON public.external_candidates FOR SELECT
  USING (
    found_by_search_id IN (
      SELECT id FROM public.headhunt_searches WHERE recruiter_id IN (
        SELECT id FROM public.recruiter_profiles WHERE user_id = auth.uid()
      )
    )
  );
...
CREATE POLICY "Recruiters can view scraping jobs for their searches"
...
CREATE TRIGGER update_headhunt_searches_updated_at
